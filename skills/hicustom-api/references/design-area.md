...
> 用途：在商品图/设计源图上，叠加一个醒目的**定制区标记**（虚线框 + "YOUR DESIGN HERE" 等文字），
...
## 流程
1. 抓空白商品详情（`product:detail`）→ 拿到商品名、印刷面数。
2. 读取源图：`--image` 指定，或 `config.inputDir`（默认 `input/`）下所有图片。
3. `DesignAreaService.mark()` 用 sharp 叠加：
   - 自动判断**明暗**（深底→白字，浅底→深字）；
   - "YOUR DESIGN HERE" 主文字（粗体、居中、字间距）+ 副文字；
   - **居中虚线圆角框**（占宽 ~72%）标记定制区，半透明衬底提高可读性。
4. 保存到 `edited/<商品ID>/`（**商品 ID 命名的文件夹**），文件名同源图。
