...
- 自动生成前后对比页：`output/stamp-compare.html`（→ http://127.0.0.1:8098/stamp-compare.html）。
  页面每张图下方带**规格行**（`宽 × 高 px · 大小`）；有原图时显示 `原图 → 裁切后` 并标注是否「已裁切」（`CompareRenderer`）。
...
- **★ 逐字符兜底（防溢出被裁）**：某个 token **自身**就超过块宽时（**中文/日文整句无空格**、超长英文串），
  自动**按字符再断行**，保证任何一行都不越界 → 文字**不会被画布裁掉**。
  例：`这是一个很长的中文标题没有空格`（显式字号）→ 逐字断成多行。
  （宽度测量带缓存，长文本也不会因重复栅格化而明显变慢。）
